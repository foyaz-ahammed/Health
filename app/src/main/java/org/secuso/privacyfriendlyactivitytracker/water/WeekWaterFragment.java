 ...
 * 주별물자료현시화면
 */
public class WeekWaterFragment extends BaseFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public WeekWaterFragment() {
        // Required empty public constructor
    }

    public static WeekWaterFragment newInstance() {
        WeekWaterFragment fragment = new WeekWaterFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        viewModel.instanceForWeek();
        viewModel.setWeekPeriod(firstDay.getTimeInMillis(), lastDay.getTimeInMillis());

        viewModel.weekData.observe(getViewLifecycleOwner(), weekData -> {
            List<WaterInfo> infoList = new ArrayList<>();
            for (int i = 0; i < weekData.size(); i ++) {
                WaterInfo info = new WaterInfo();
                info.setId(weekData.get(i).getId());
                info.setGlasses(weekData.get(i).getGlasses());
                DateTime dateTime = new DateTime(weekData.get(i).getDate());
