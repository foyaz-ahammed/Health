...
public class WaveView extends View {
    ...
    public static final int DEFAULT_BORDER_COLOR = Color.parseColor("#FFFFFF");
    ...
    public WaveView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        CornerPathEffect corEffect = new CornerPathEffect(50.0f);
        mBorderPaint.setPathEffect(corEffect);
        mBorderPaint.setStrokeCap(Paint.Cap.ROUND);
        mViewPaint.setPathEffect(corEffect);
    ...
    private Path drawSquare(int cx, int cy, int radius) {
        Path path = new Path();
        path.moveTo(cx + 40, cy + (mBorderWidth));
        path.lineTo(cx + 100, radius + cy - mBorderWidth);
        path.lineTo(radius + cx - 100, radius + cy - mBorderWidth);
        path.lineTo(radius + cx - 40, cy + mBorderWidth);
        return path;
    }
     ...
     * @param cy          Y
     * @param spikes      별의 각도수
     * @param outerRadius 외부원반경
     * @param innerRadius 내부원반경
     * @return 통로
     */
    ...
    /**
     * 채우기 shader 만들기
     * y = Asin(ωx+φ)+h wave 공식 (함수) y = waveLevel * Math.sin(w * x1 + shiftX) + level
     * φ (초기단계 x)：Wave x축 offset     $shiftX
     * ω (각 주파수)：최소기간 T=2π/|ω|       $w
     * A (진폭)：혹크기     $waveLevel
     * h (초기단계 y)：Wave 류형 y축 offset     $level
     * <p>
     * Bezier 곡선(Bézier curve)
    ...
    private void createShader() {
        ...
        float level = ((((float) (mMax - mProgress)) / (float) mMax) * viewSize) + ((screenSize.y / 2) - (viewSize / 2)); //물높이
        int x2 = viewSize + 1;
        int y2 = viewSize + 1;
        float zzz = (((float) viewSize * ((waveOffset - 50) / 100f)) / ((float) viewSize / 6.25f));
        float shiftX2 = shiftX1 + zzz; //wave 차이
